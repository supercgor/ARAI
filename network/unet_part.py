    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, apply_pooling=True,
        ...
        if apply_pooling:
            if all_dim == 1:
                ...
                z_size = 1
            if pool_type == 'max':
                self.pooling = nn.MaxPool3d(kernel_size=(z_size, pool_kernel_size, pool_kernel_size))
            else:
                self.pooling = nn.AvgPool3d(kernel_size=(z_size, pool_kernel_size, pool_kernel_size))
        else:
            self.pooling = None
...
class Out(nn.Module):
    def __init__(self, in_channels=32, out_channels=8, kernel_size=3):
        ...
        self.out = nn.Sequential(
            nn.Conv3d(in_channels, 2 * in_channels, kernel_size, padding=1, padding_mode='replicate'),
            nn.LeakyReLU(inplace=True),
            nn.Conv3d(2 * in_channels, 2 * in_channels, kernel_size, padding=1, padding_mode='replicate'),
            nn.LeakyReLU(inplace=True),
            nn.MaxPool3d((1, 2, 2), stride=(1, 2, 2)),
            nn.Conv3d(2 * in_channels, 4 * in_channels, kernel_size, padding=1, padding_mode='replicate'),
            nn.LeakyReLU(inplace=True),
            nn.Conv3d(4 * in_channels, 4 * in_channels, kernel_size, padding=1, padding_mode='replicate'),
            nn.LeakyReLU(inplace=True),
            nn.MaxPool3d((1, 2, 2), stride=(1, 2, 2)),
            nn.Conv3d(4 * in_channels, 2 * in_channels, 1, padding=0, padding_mode='replicate'),
            nn.LeakyReLU(inplace=True),
            nn.Conv3d(2 * in_channels, out_channels, 1, padding=0, padding_mode='replicate'),
        )
