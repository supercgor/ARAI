...
class Trainer():
    ...
    def train_one_epoch(self, epoch, log_every: int = 25) -> tuple[torch.Tensor]:
        ...
        for i, (filenames, inps, _, embs) in enumerate(self.TrainLoader):
            inps = inps.to(self.device, non_blocking = True)
            embs = embs.to(self.device, non_blocking = True)
            preds, mu, var = self.model(inps, embs)
            
            loss_conf, loss_pos, loss_r, loss_vae = self.Criterion(preds, inps, mu, var)
    ...
    def test_one_epoch(self, epoch, log_every: int = 25) -> tuple[torch.Tensor]:
        ...
        self.ConfusionCounter.reset()
        self.RotStat.reset()
        for i, (filenames, inps, _, embs) in enumerate(self.TestLoader):
            inps = inps.to(self.device)
            embs = embs.to(self.device)
            preds, mu, logvar = self.model(inps, embs)
            
            loss_wc, loss_pos, loss_r, loss_vae = self.Criterion(preds, inps, mu, logvar)
...
@hydra.main(config_path="conf", config_name="4a4a", version_base=None) # hydra will automatically relocate the working dir.
def main(cfg):
    rank = 0
