import os
import numpy as np
from utils.basic import read_POSCAR
...
class Analyzer(torch.nn.Module):
    def __init__(self, cfg):
        ...
        self.register_buffer('lattice_expand', self.real_size/torch.tensor(self.space_size))
    ...
    def count(self, info):
        ...
        TP_index, P_pos, T_pos = info["TP_index"], info["P_pos"], info["T_pos"]
    ...
    def to_poscar(self, predictions, filenames, out_dir, nms=True):
        ...
        for batch in range(batch_size):
            for position_list, filename in zip(P_pos,filenames):
                ...
                try:
                    os.mkdir(os.path.join(out_dir, self.cfg.TRAIN.CHECKPOINT))
                except:
                    pass
                file_path = os.path.join(out_dir, self.cfg.TRAIN.CHECKPOINT,filename + '.poscar')
